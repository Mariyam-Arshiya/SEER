...
public class SensitiveDataDetector {
  ...
  private boolean isPartOfLongerNumericSequence(String text,int start,int end){ int left=start, right=end; while(left>0 && (Character.isDigit(text.charAt(left-1)) || isNumericSeparator(text.charAt(left-1)))) left--; while(right<text.length() && (Character.isDigit(text.charAt(right)) || isNumericSeparator(text.charAt(right)))) right++; int digits=0; for(int i=left;i<right;i++){ if(Character.isDigit(text.charAt(i))) digits++; } return digits>12; }
  private boolean isNumericSeparator(char ch){ return ch==' ' || ch=='-' || ch=='/' || ch=='_' || ch=='.'; }
